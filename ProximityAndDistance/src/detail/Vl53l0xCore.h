...
#ifndef MYOSA_VL53L0X_CORE_H
#define MYOSA_VL53L0X_CORE_H
...
class Vl53l0xCore
{
  ...
  private:
    friend class ::ProximityAndDistance;
    bool io_error = false;
    bool expired();
    ...
    uint8_t
        stop_variable; // read by init and used when starting measurement; is StopVariable field of Vl53l0xCore_DevData_t structure in API
    ...
    static uint32_t decodeTimeout(uint16_t value);
    static uint16_t encodeTimeout(uint32_t timeout_mclks);
    static uint32_t timeoutMclksToMicroseconds(uint32_t timeout_period_mclks,
